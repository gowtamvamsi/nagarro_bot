...
class DialogPromptBot {
    ...
    constructor(conversationState) {
        ...
        this.dialogStateAccessor = conversationState.createProperty(
            DIALOG_STATE_ACCESSOR
        );
        this.reservationAccessor = conversationState.createProperty(
            RESERVATION_ACCESSOR
        );
        ...
        this.dialogSet.add(
            new NumberPrompt(PARTY_SIZE_PROMPT, this.partySizeValidator)
        );
        this.dialogSet.add(new ChoicePrompt(LOCATION_PROMPT));
        this.dialogSet.add(
            new DateTimePrompt(RESERVATION_DATE_PROMPT, this.dateValidator)
        );

        // Define the steps of the waterfall dialog and add it to the set.
        this.dialogSet.add(
            new WaterfallDialog(RESERVATION_DIALOG, [
    ...
    async promptForPartySize(stepContext) {
        ...
        return await stepContext.prompt(PARTY_SIZE_PROMPT, {
    ...
    async promptForLocation(stepContext) {
        ...
        // Prompt for location
        return await stepContext.prompt(LOCATION_PROMPT, 'Select a location.', [
            'Gurgaon',
            'Delhi',
            'Noida'
        ]);
    ...
    async promptForReservationDate(stepContext) {
        // Prompt for the party size. The result of the prompt is returned to the next step of the waterfall.
        return await stepContext.prompt(RESERVATION_DATE_PROMPT, {
    ...
    async onTurn(turnContext) {
        ...
        case ActivityTypes.Message:
            ...
            const reservation = await this.reservationAccessor.get(
                turnContext,
                null
            );
...
module.exports.MyBot = DialogPromptBot;
