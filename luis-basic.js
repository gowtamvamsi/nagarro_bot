 ...
 */
class BasicBot {
    constructor(application, conversationState, userState) {
    ...
    async isTurnInterrupted(dc, luisResults) {
        ...
        if (topIntent === CANCEL_INTENT) {
            if (dc.activeDialog) {
                ...
                await dc.cancelAllDialogs();
                await dc.context.sendActivity(
                    `Ok.  I've cancelled our last activity.`
                );
            } else {
                await dc.context.sendActivity(
        ...
        if (Object.keys(luisResult.entities).length !== 1) {
            ...
            USER_LOCATION_ENTITIES.forEach(city => {
                if (luisResult.entities[city] !== undefined) {
                    ...
                    // capitalize and set user name
                    userProfile.city =
                        lowerCaseCity.charAt(0).toUpperCase() +
                        lowerCaseCity.substr(1);
                }
            });
            // set the new values
